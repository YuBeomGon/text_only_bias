...
| exp03 | [논문 충실 재현](./exp03_paper_faithful/design.md) | **E_pretrained 초기화(#1) + decoder fine-tune(#3)**, kv-cache 레벨 주입 | ⏳ 설계 완료, 미착수 | — |
...
2. **사후 논문 정독**: 논문의 "K/V bias"는 encoder-output 대체(=exp01)이며, 우리는 **①B 초기화=E_pretrained,
   ②decoder fine-tune**를 놓침. → 음성 결과는 "방식 불가"보다 "**논문 미준수**" 쪽. 자세히는 CONCLUSIONS.md §4.
3. 모든 실험은 **baseline sanity(bias off == plain Whisper) 토큰단위 일치**를 먼저 통과해야 함 (전부 통과).
4. 다음: **exp03(논문 충실)** → 그래도 안 되면 shallow fusion.
